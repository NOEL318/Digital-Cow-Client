...
export function DestetarFlow() {
  ...
  async function save() {
    ...
    try {
      ...
      setError((e as { response?: { data?: { error?: { message?: string } } } })
        ?.response?.data?.error?.message ?? 'No pudimos guardar el destete.');
    }
  }

  if (step === 1) {
    return (
      <WizardStep current={1} total={3} title="¿A cuál becerro destetaste?"
  ...
  if (step === 2) {
    return (
      <WizardStep current={2} total={3} title="¿Cuándo y a qué peso?"
        canAdvance={!!weanedAt}
        onNext={() => setStep(3)} onBack={() => setStep(1)}>
        <HelpfulField id="wn-date" label="Fecha del destete" icon={Calendar} required>
        ...
        <HelpfulField id="wn-weight" label="Peso al destete (kg)" icon={Scale} example="180">
  ...
  return (
    <WizardStep current={3} total={3} title="¿Listo? Así se guardará"
      ...
      <div className="rounded-xl border p-4 space-y-3">
        ...
        <p className="flex items-center gap-2"><GraduationCap className="h-4 w-4 text-primary" aria-hidden /> Destetado</p>
