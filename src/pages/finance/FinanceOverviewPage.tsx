...
export default function FinanceOverviewPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* KPI cards with semantic colors and glassmorphic gradients */}
      <div className="grid gap-3 grid-cols-1 md:grid-cols-3">
        <Card className="glass-gradient-emerald border-l-4 border-l-emerald-600">
          ...
          <CardContent className="text-3xl font-black text-emerald-700 dark:text-emerald-400">
            ${totals ? Number(totals.totalIncome).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}
          </CardContent>
        </Card>
        <Card className="glass-gradient-rose border-l-4 border-l-rose-600">
          ...
          <CardContent className="text-3xl font-black text-rose-700 dark:text-rose-400">
            ${totals ? Number(totals.totalExpense).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}
          </CardContent>
        </Card>
        <Card className={`border-l-4 ${margin !== null && margin >= 0
          ? 'glass-gradient-blue border-l-blue-600'
          : 'glass-gradient-amber border-l-amber-600'}`}>
          <CardHeader>
            <CardTitle className={`text-xs font-bold uppercase tracking-wider ${margin !== null && margin >= 0
              ? 'text-blue-700 dark:text-blue-400'
              : 'text-amber-700 dark:text-amber-400'}`}>
              {t('finance:overview.ytdMargin')}
            </CardTitle>
          </CardHeader>
          <CardContent className={`text-3xl font-black ${margin !== null && margin >= 0
            ? 'text-blue-700 dark:text-blue-400'
            : 'text-amber-700 dark:text-amber-400'}`}>
            ${totals ? Number(totals.margin).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}
          </CardContent>
        </Card>
      </div>
